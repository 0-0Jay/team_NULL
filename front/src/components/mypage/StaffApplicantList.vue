<!-- /component/mypage/StaffApplicantList.vue -->
<script setup>
...
const groupedApplicant = computed(() => {
  return {
    direct: applicant.value.filter((a) => a.type === 'direct'),
    assigned: applicant.value.filter((a) => a.type === 'assigned')
  };
});
...
<template>
  <div class="card flex flex-col gap-4 h-full">
    ...
    <div v-if="applicant.length" class="flex-1 overflow-y-auto">
      <ul>
        <!-- 직접 등록한 지원자 -->
        <li v-if="groupedApplicant.direct.length" class="flex items-center gap-3 text-m text-blue-500 font-semibold my-3">
          <span class="whitespace-nowrap">직접 등록</span>
          <div class="flex-1 border-t-4 border-blue-300"></div>
        </li>
        <li
          v-for="app in groupedApplicant.direct"
          :key="app.a_no"
        ...
        <!-- 신청서 배정 지원자 -->
        <li v-if="groupedApplicant.direct.length" class="flex items-center gap-3 text-m text-blue-500 font-semibold mt-10 mb-3">
          <span class="whitespace-nowrap">신청서 배정</span>
          <div class="flex-1 border-t-4 border-blue-300"></div>
        </li>
        <li
          v-for="app in groupedApplicant.assigned"
          :key="app.a_no"
          class="text-lg cursor-pointer pt-2 pr-2"
          :class="{ 'border-l-5 pl-3 pt-2 border-blue-500': route.name === 'myPageApplicantDetail' && Number(route.params.a_no) === app.a_no }"
          @click="selectedApplicant(app.a_no)"
