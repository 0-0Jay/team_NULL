...
const submitEdit = async () => {
  ...
  if (user.type === 2) {
    // 기관 관리자: 로그인 정보에서 c_no 사용
    cNoValue = user.c_no;
  } else {
    // 시스템 관리자: 선택값이 있으면 selectedAutoValue, 없으면 editUser.c_no
    cNoValue = selectedAutoValue?.value?.c_no ?? editUser.value.c_no;
  }
  ...
  if (user.type === 3) {
    ...
    msg.value.centerName = false; // 기관 관리자면 체크 제외
  }
          ...
          <td class="py-2 px-2">
            <IconField iconPosition="left">
              ...
              <AutoComplete
                ...
                @item-select="
                  () => {
                    if (msg.value) msg.value.centerName = false;
                  }
                "
              />
            </IconField>
