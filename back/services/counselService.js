...
const addCounsel = async ({
  ...
  counsel_author
    ...
    let list = await mysql.query(
        "insertCounsel",
        [application_no, title, content, counselDate, counsel_author],
        "counsel"
    );
    return list;
};
...
// 작성된 상담내역 조회
const findCounsel = async (application_no) => {
    let list = await mysql.query("selectCounsel", [application_no], "counsel");
    return list;
};
